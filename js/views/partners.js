...
const partnersView = {
  ...
  async afterRender() {
    // Načítaj len ak ešte nemáme dáta
    if (!this._loaded) {
  ...
  _setFilter(f) {
    this._roleFilter = f;
    // Aktualizuj len tlačidlá — bez re-renderu celého view
  ...
  // Invaliduj cache pri zmene roly
  _invalidate() {
    this._loaded   = false;
    this._partners = [];
  },
  ...
  openInvite() {
    modal.open('Pozvať užívateľa', `
      <div style="font-size:13px;color:var(--muted);margin-bottom:16px;">
        Užívateľ dostane email s odkazom na prihlásenie.
      </div>
      <div class="form-row"><label class="form-label">Email *</label>
        <input id="inv-email" type="email" placeholder="uzivatel@email.sk" /></div>
      <div class="form-row"><label class="form-label">Meno</label>
        <input id="inv-name" placeholder="Ján Novák" /></div>
      <div class="form-row">
