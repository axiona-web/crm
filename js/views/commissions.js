...
const commissionsView = {
    ...
    return `
      ...
      <div class="comm-stats">
        <div class="card stat-card">
          ...
          <div class="stat-value mono" style="color:var(--acc);font-size:19px;">${EUR(pending)}</div>
        </div>
        <div class="card stat-card">
          <div class="stat-label">Vyplatené</div>
          <div class="stat-value mono" style="color:var(--green);font-size:19px;">${EUR(paid)}</div>
        </div>
        <div class="card stat-card">
          <div class="stat-label">Celkom</div>
          <div class="stat-value mono" style="font-size:19px;">${EUR(pending + paid)}</div>
        </div>
      </div>
  ...
  toggle(id) {
    app.state.commissions = app.state.commissions.map(c =>
      c.id === id ? { ...c, status: c.status === 'Čakajúca' ? 'Vyplatená' : 'Čakajúca' } : c
    );
    db.set('commissions', app.state.commissions);
    app.renderContent();
  },
  ...
  _form(c, isNew) {
    ...
    const dOpts = app.state.deals.map(x =>
      `<option value="${x.id}"${c.dealId === x.id ? ' selected' : ''}>${esc(x.name)}</option>`
    ).join('');
    return `
      ...
      <div class="form-row">
        ...
        <select id="cm-contact"><option value="">— vybrať —</option>${cOpts}</select>
      </div>
      <div class="form-row">
        <label class="form-label">Obchod</label>
        <select id="cm-deal"><option value="">— vybrať —</option>${dOpts}</select>
      </div>
      <div class="form-grid-2">
        <div class="form-row">
      ...
      <div class="form-row"><label class="form-label">Poznámky</label><textarea id="cm-notes" style="min-height:55px;resize:vertical;">${esc(c.notes || '')}</textarea></div>
  ...
  openAdd() {
    modal.open('Nová provízia', this._form({
      dealId: '', contactId: '', amount: '', rate: '',
      status: 'Čakajúca', date: new Date().toISOString().slice(0, 10), notes: '',
    }, true));
  },

  openEdit(id) {
    const c = app.state.commissions.find(x => x.id === id);
    if (c) modal.open('Upraviť províziu', this._form(c, false));
  },
  ...
  save(id, isNew) {
    ...
    if (isNew) {
      app.state.commissions.push({ ...obj, id: uid(), createdAt: new Date().toISOString() });
    } else {
      app.state.commissions = app.state.commissions.map(c => c.id === id ? { ...c, ...obj } : c);
    }
    db.set('commissions', app.state.commissions);
    modal.close();
    app.renderContent();
