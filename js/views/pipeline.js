...
const pipelineView = {
  ...
  render() {
    ...
    const groups = STAGES.map(s=>({
      s, cnt: deals.filter(d=>d.stage===s).length,
      val: deals.filter(d=>d.stage===s).reduce((a,d)=>a+(d.value||0),0)
    }));
    const filtered = deals.filter(d=>this.filter==='Všetky'||d.stage===this.filter)
    ...
    return `
      ...
      <div class="stage-filters">
        ...
        ${this.filter!=='Všetky'?`<button class="btn-ghost" style="align-self:center;font-size:12px;padding:5px 11px;" onclick="pipelineView.filter='Všetky';app.renderContent();">× Všetky</button>`:''}
      </div>
      ${filtered.length===0
              ...
              <div class="card">
                <div class="list-item">
                  ...
                  <div class="deal-actions">
                    <button class="icon-btn" onclick="pipelineView.move('${d.id}',-1)">◀</button>
                    <button class="icon-btn" onclick="pipelineView.move('${d.id}',1)">▶</button>
  ...
  async move(id, dir) {
    ...
    const updated = await db.updateDeal(id, {...d, stage: STAGES[idx]});
  ...
  _form(d, isNew) {
    ...
    return `
      ...
      <div class="form-row"><label class="form-label">Kontakt</label>
        <select id="df-contact"><option value="">— vybrať —</option>${opts}</select></div>
      <div class="form-row"><label class="form-label">Fáza</label>
        <select id="df-stage">${STAGES.map(s=>`<option${d.stage===s?' selected':''}>${s}</option>`).join('')}</select></div>
      ...
      <div class="form-row"><label class="form-label">Poznámky</label>
        <textarea id="df-notes" style="min-height:60px;resize:vertical;">${esc(d.notes||'')}</textarea></div>
  ...
  openAdd()    { modal.open('Nová príležitosť', this._form({name:'',contactId:'',value:'',stage:'Kontakt',probability:20,expectedClose:'',notes:''}, true)); },
  openEdit(id) { const d=app.state.deals.find(x=>x.id===id); if(d) modal.open('Upraviť príležitosť', this._form(d, false)); },
