...
const contactsView = {
                ...
                <div class="list-item">
                  <div>
                    <div style="display:flex;align-items:center;gap:7px;margin-bottom:4px;">
                      ...
                      ${badge(c.type, TYPE_COLORS[c.type] || 'var(--muted)')}
                    </div>
  ...
  _form(c, isNew) {
    return `
      ...
      <div class="form-row">
        ...
        <textarea id="cf-notes" style="min-height:65px;resize:vertical;">${esc(c.notes || '')}</textarea>
      </div>
  ...
  openAdd() {
    modal.open('Nový kontakt', this._form({ name: '', company: '', phone: '', email: '', type: 'Klient', notes: '' }, true));
  },
  ...
  save(id, isNew) {
    ...
    if (!obj.name) return;
    if (isNew) {
      app.state.contacts.push({ ...obj, id: uid(), createdAt: new Date().toISOString() });
    } else {
      app.state.contacts = app.state.contacts.map(c => c.id === id ? { ...c, ...obj } : c);
    }
    db.set('contacts', app.state.contacts);
    modal.close();
    app.updateFooter();
    app.renderContent();
  },
